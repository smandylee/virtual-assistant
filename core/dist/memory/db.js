...
exports.saveLearningContext = saveLearningContext;
exports.getLearningContext = getLearningContext;
exports.learnConversationPattern = learnConversationPattern;
exports.getConversationPatterns = getConversationPatterns;
...
exports.cleanupOldInteractions = cleanupOldInteractions;
exports.cleanupLowImportanceContext = cleanupLowImportanceContext;
exports.cleanupDuplicatePatterns = cleanupDuplicatePatterns;
exports.getDatabaseStats = getDatabaseStats;
exports.autoCleanup = autoCleanup;
exports.saveFeedback = saveFeedback;
exports.updatePolicyScore = updatePolicyScore;
exports.getPolicyScores = getPolicyScores;
exports.saveKnowledgeCard = saveKnowledgeCard;
exports.searchKnowledgeCards = searchKnowledgeCards;
exports.incrementKnowledgeCardUsage = incrementKnowledgeCardUsage;
exports.getFeedbackStats = getFeedbackStats;
exports.markOpened = markOpened;
exports.isRecentlyOpened = isRecentlyOpened;
...
let db;
function initDb() {
    fs_1.default.mkdirSync("./data/db", { recursive: true });
...
function logInteraction(user_text, model_text) {
    const stmt = db.prepare(`INSERT INTO interactions(user_text, model_text) VALUES (?, ?)`);
    const info = stmt.run(user_text, model_text);
    return info.lastInsertRowid;
}
// 사용자 선호도 저장
function saveUserPreference(key, value) {
    const stmt = db.prepare(`INSERT OR REPLACE INTO user_preferences(key, value, updated_at) VALUES (?, ?, datetime('now', 'localtime'))`);
    stmt.run(key, value);
}
// 사용자 선호도 조회
...
// 학습 컨텍스트 저장
function saveLearningContext(type, data, importance = 1) {
    const stmt = db.prepare(`INSERT INTO learning_context(context_type, context_data, importance) VALUES (?, ?, ?)`);
    stmt.run(type, data, importance);
}
// 학습 컨텍스트 조회
function getLearningContext(type, limit = 10) {
...
// 대화 패턴 학습
function learnConversationPattern(userPattern, preferredResponse) {
    const stmt = db.prepare(`INSERT OR IGNORE INTO conversation_patterns(user_pattern, preferred_response) VALUES (?, ?)`);
    stmt.run(userPattern, preferredResponse);
    // 빈도 업데이트
    const updateStmt = db.prepare(`UPDATE conversation_patterns SET frequency = frequency + 1 WHERE user_pattern = ? AND preferred_response = ?`);
    updateStmt.run(userPattern, preferredResponse);
}
// 대화 패턴 조회
function getConversationPatterns(userPattern) {
    const stmt = db.prepare(`SELECT preferred_response, frequency FROM conversation_patterns 
     WHERE user_pattern LIKE ? ORDER BY frequency DESC LIMIT 5`);
    return stmt.all(`%${userPattern}%`);
}
// 최근 대화 기록 조회
function getRecentInteractions(limit = 5) {
    const stmt = db.prepare(`SELECT user_text, model_text FROM interactions 
...
function backupDatabase(backupPath) {
    ...
    // 백업 디렉토리 생성
    const backupDir = path_1.default.dirname(finalBackupPath);
    fs_1.default.mkdirSync(backupDir, { recursive: true });
    // 데이터베이스 파일 복사
    fs_1.default.copyFileSync(DB_PATH, finalBackupPath);
...
function getBackupList() {
    ...
    return fs_1.default.readdirSync(backupDir)
        ...
        .map(file => path_1.default.join(backupDir, file))
        .sort((a, b) => {
...
// ==================== 기억 관리 기능 ====================
// 오래된 대화 기록 삭제 (30일 이상)
function cleanupOldInteractions(daysToKeep = 30) {
    // SQL 인젝션 방지: 파라미터 바인딩 사용
    const stmt = db.prepare(`DELETE FROM interactions 
...
// 중요도가 낮은 학습 컨텍스트 삭제
function cleanupLowImportanceContext(importanceThreshold = 1) {
    // SQL 인젝션 방지: 파라미터 바인딩 사용
    const stmt = db.prepare(`DELETE FROM learning_context 
...
function cleanupDuplicatePatterns() {
...
function getDatabaseStats() {
    const stats = {
        interactions: db.prepare('SELECT COUNT(*) as count FROM interactions').get(),
        preferences: db.prepare('SELECT COUNT(*) as count FROM user_preferences').get(),
        learningContext: db.prepare('SELECT COUNT(*) as count FROM learning_context').get(),
        patterns: db.prepare('SELECT COUNT(*) as count FROM conversation_patterns').get(),
        databaseSize: fs_1.default.statSync(DB_PATH).size
    };
    return {
        ...stats,
...
// ==================== 피드백 시스템 ====================
// 피드백 저장
function saveFeedback(interactionId, feedbackType, feedbackTags = [], correctedText, userRating) {
    const stmt = db.prepare(`
...
// 정책 점수 업데이트
function updatePolicyScore(toolName, intent, success) {
    const score = success ? 1 : -1;
    // 기존 점수 조회
    const existing = db.prepare(`SELECT * FROM tool_policy_scores WHERE tool_name = ? AND intent = ?`).get(toolName, intent);
    if (existing) {
        // 기존 점수 업데이트
...
// 정책 점수 조회
function getPolicyScores(intent) {
...
// 지식 카드 저장
function saveKnowledgeCard(question, answer, context, tags = []) {
    const stmt = db.prepare(`
...
// 지식 카드 검색
function searchKnowledgeCards(query, limit = 5) {
    const stmt = db.prepare(`
...
// 지식 카드 사용 횟수 증가
function incrementKnowledgeCardUsage(cardId) {
    const stmt = db.prepare(`
...
function getFeedbackStats() {
    const stats = {
        totalFeedback: db.prepare('SELECT COUNT(*) as count FROM feedback_logs').get(),
        thumbsUp: db.prepare('SELECT COUNT(*) as count FROM feedback_logs WHERE feedback_type = "thumbs_up"').get(),
        thumbsDown: db.prepare('SELECT COUNT(*) as count FROM feedback_logs WHERE feedback_type = "thumbs_down"').get(),
        corrections: db.prepare('SELECT COUNT(*) as count FROM feedback_logs WHERE feedback_type = "correction"').get(),
        averageRating: db.prepare('SELECT AVG(user_rating) as avg FROM feedback_logs WHERE user_rating IS NOT NULL').get()
    };
    return stats;
}
// ==================== 열림 상태 추적 ====================
...
function normalizePath(pathStr) {
    ...
    return path_1.default.normalize(pathStr).toLowerCase().replace(/\\/g, '/');
}
function markOpened(pathStr, kind) {
...
function isRecentlyOpened(pathStr, withinMinutes = 5) {
    // 경로 정규화: 대소문자 통일, 슬래시 정규화
    const normalized = normalizePath(pathStr);
    const row = db.prepare(`SELECT last_opened FROM opened_items WHERE path = ?`).get(normalized);
    if (!row?.last_opened)
        return false;
