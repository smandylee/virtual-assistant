...
function inAllow(p: string) {
  ...
  const result = allAllowedPaths.some(allowedPath => {
    const resolvedPath = path.resolve(allowedPath).replace(/\\/g,"/");
...
function isCommandSafe(command: string): { safe: boolean; reason?: string } {
  ...
  const isAllowed = ALLOWED_COMMANDS.some(allowed => 
    firstWord === allowed || cmd.startsWith(allowed + ' ')
  );
...
// 명령어 실행 함수
async function executeCommand(command: string, timeout: number = 30000): Promise<{
  success: boolean;
  output: string;
  error?: string;
  exitCode?: number;
...
export const tools = {
  search_files: {
    schema: z.object({ 
      query: z.string(), 
      dir: z.string().default(ALLOW_DIR),
    ...
    async execute({ query, dir, maxResults = 50, recursive = false }: { 
      ...
      async function searchDir(currentDir: string, depth: number = 0) {
        if (results.length >= maxResults || depth > 2) return; // 최대 2단계 depth만
  ...
  execute_command: {
    ...
    async execute({ command, timeout }: { command: string; timeout: number }) {
      // 안전성 검사
  ...
  update_policy_score: {
    ...
    async execute({ tool, intent, success }: { tool: string; intent: string; success: boolean }) {
  ...
  open_folder: {
    ...
    async execute({ path: folderPath }: { path: string }) {
      // 보안 검사
      ...
      if (isRecentlyOpened(folderPath, 2)) {
        return {
          folder: folderPath,
          success: true,
  ...
  open_file: {
    ...
    async execute({ path: filePath }: { path: string }) {
      ...
      // 이미 최근에 열렸는지 확인 (중복 실행 방지)
      if (isRecentlyOpened(filePath, 2)) {
        return {
          file: filePath,
          success: true,
          message: "이미 열려있거나 방금 열렸습니다",
  ...
  open_with: {
    ...
    async execute({ filePath, program }: { filePath: string; program: string }) {
      // 보안 검사
      if (!inAllow(filePath)) {
        throw new Error("허용되지 않은 경로입니다");
      }
  ...
  add_schedule: {
    ...
    async execute({ title, date, time, description = "" }: { 
      ...
      try {
        ...
        const scheduleFile = path.join(process.cwd(), 'data', 'schedules.json');
        
        // 기존 스케줄 로드
        let schedules = [];
        try {
          const existingData = await fs.readFile(scheduleFile, 'utf-8');
  ...
  get_schedules: {
    schema: z.object({
      date: z.string().optional(), // 특정 날짜 조회 (YYYY-MM-DD)
      upcoming: z.boolean().optional().default(false) // 다가오는 일정만 조회
    }),
    async execute({ date, upcoming = false }: { date?: string; upcoming?: boolean }) {
      try {
        const scheduleFile = path.join(process.cwd(), 'data', 'schedules.json');
    ...
    async execute() {
      try {
        ...
        if (expired.length > 0) {
          ...
          await fs.writeFile(scheduleFile, JSON.stringify(activeSchedules, null, 2));
        }
  ...
  launch_steam_game: {
    schema: z.object({
      gameId: z.string().optional(), // 스팀 게임 ID (AppID)
      gameName: z.string().optional() // 게임 이름 (게임 ID를 모를 경우)
    }),
    async execute({ gameId, gameName }: { gameId?: string; gameName?: string }) {
      try {
        // 게임 ID가 있으면 직접 실행
        if (gameId) {
        ...
        if (gameName) {
          ...
          for (const installPath of steamInstallPaths) {
            try {
              const defaultLibrary = path.join(installPath, 'steamapps');
              await fs.access(defaultLibrary);
              steamLibraryPaths.push(defaultLibrary);
              console.log(`기본 라이브러리 발견: ${defaultLibrary}`);
              
              // libraryfolders.vdf 파일 읽기 (추가 라이브러리 경로 찾기)
              const libraryFoldersPath = path.join(defaultLibrary, 'libraryfolders.vdf');
              try {
                const libraryFoldersContent = await fs.readFile(libraryFoldersPath, 'utf-8');
                // libraryfolders.vdf에서 경로 추출
                const pathMatches = libraryFoldersContent.matchAll(/"path"\s+"([^"]+)"/g);
                for (const match of pathMatches) {
                  const libraryPath = path.join(match[1], 'steamapps');
                  try {
                ...
                try {
                  ...
                  if (appIdMatch && nameMatch) {
                    ...
                    try {
                      ...
                      const folders = await fs.readdir(commonPath);
                      // AppID로 폴더 찾기 (일부 게임은 AppID로 폴더명이 지정됨)
                      // 또는 installedName으로 폴더 찾기
                      const matchingFolder = folders.find(folder => {
                        const folderPath = path.join(commonPath, folder);
                    ...
                    if (gameKoreanWords.length > 0) {
                      ...
                      if (allKoreanWords.length > 0) {
                        const anyWordMatch = gameKoreanWords.some(gw => 
                          allKoreanWords.some(iw => iw.includes(gw) || gw.includes(iw))
                        );
                      ...
                      if (mappedEnglishWords) {
                        ...
                        const installedEnglishWords: string[] = [];
                        for (const nameToCheck of allNames) {
                          const words = (nameToCheck.match(englishWordRegex) || []).map(w => w.trim()).filter(w => w.length >= 2);
                          installedEnglishWords.push(...words);
                        }
                        
                        const allMappedWordsMatch = mappedEnglishWords.every(mw => 
                          installedEnglishWords.some(iw => iw.includes(mw) || mw.includes(iw))
                        );
                    ...
                    const installedEnglishWords2: string[] = [];
                    for (const nameToCheck of allNames) {
                    ...
                    if (gameEnglishWords2.length > 0 && installedEnglishWords2.length > 0) {
                      const englishMatch = gameEnglishWords2.some(gw => 
                        installedEnglishWords2.some(iw => iw.includes(gw) || gw.includes(iw))
                      );
  ...
  youtube_play: {
    ...
    async execute({ query, videoId, url }: { query?: string; videoId?: string; url?: string }) {
      try {
        let videoUrl = '';
        let videoTitle = '';
  ...
  youtube_trending: {
    schema: z.object({
      regionCode: z.string().optional().default('KR'),
      maxResults: z.number().optional().default(10),
      category: z.string().optional() // music, gaming, news 등
    }),
  ...
  youtube_video_info: {
    ...
    async execute({ videoId }: { videoId: string }) {
      const YOUTUBE_API_KEY = process.env.YOUTUBE_API_KEY || process.env.GEMINI_API_KEY;
  ...
  launch_program: {
    ...
    async execute({ programName }: { programName: string }) {
      try {
        ...
        const foundPrograms: Array<{ name: string; path: string; type: string }> = [];
        
        // 매칭 함수 (내부 함수들이 접근할 수 있도록 execute 함수 스코프에 정의)
        const matchesAnyName = (target: string, candidates: string[]): boolean => {
        ...
        const searchExeFiles = async (
          dirPath: string,
          searchNames: string[],
          results: Array<{ name: string; path: string; type: string }>,
          depth: number = 0
        ) => {
          if (depth > 3) return; // Program Files는 깊이 제한
        ...
        // 레지스트리에서 설치된 프로그램 검색
        const searchRegistryPrograms = async (searchNames: string[]): Promise<Array<{ name: string; path: string; type: string }>> => {
                          ...
                          if (installMatch) {
                            ...
                            try {
                              ...
                              if (exeFile) {
                                results.push({
                                  name: displayName,
                                  path: path.join(installPath, exeFile),
                                  type: 'executable'
                                });
                              }
                            } catch {
        ...
        if (bestMatch) {
          ...
          try {
            ...
            if (bestMatch.path.endsWith('.lnk')) {
              ...
              const child = spawn('cmd', ['/c', 'start', '""', bestMatch.path], {
                shell: false,
                detached: true,
                stdio: 'ignore',
              ...
              const child = spawn(bestMatch.path, [], {
                shell: false,
                detached: true,
                stdio: 'ignore'
              });
  ...
  analyze_document: {
    ...
    schema: z.object({
      ...
      outputFormat: z.enum(["text", "summary", "detailed"]).optional().default("text")
    }),
    async execute({ filePath, outputFormat = "text" }: { filePath: string; outputFormat?: string }) {
      try {
  ...
  copy_to_notepad: {
    ...
    schema: z.object({
      text: z.string().describe("클립보드에 복사할 텍스트"),
      openEditor: z.boolean().optional().default(true)
    }),
    async execute({ text, openEditor = true }: { text: string; openEditor?: boolean }) {
      try {
        ...
        if (openEditor) {
          if (isWindows) {
            ...
            spawn('open', ['-a', 'TextEdit'], { detached: true, stdio: 'ignore' }).unref();
          } else {
            // Linux: gedit 또는 xdg-open
            spawn('gedit', [], { detached: true, stdio: 'ignore' }).unref();
          }
