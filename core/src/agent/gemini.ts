...
// 컨텍스트 최적화 함수
const clip = (s: string, n: number = 300) => s.length > n ? s.slice(0, n) + '…' : s;

// ==================== 새로운 주제 감지 ====================
function isNewTopic(currentMsg: string, previousMsgs: string[]): boolean {
  if (previousMsgs.length === 0) return true;
  ...
  const hasOverlap = currentKeywords.some(kw => 
    lastInteraction.includes(kw) && kw.length > 2
  );
...
// ==================== 메인 채팅 함수 (맥락 강화) ====================
export async function chatWithGemini(system: string, user: string) {
  if (!GEMINI_API_KEY) {
    throw new Error("GEMINI_API_KEY가 설정되지 않았습니다. .env 파일에 GEMINI_API_KEY를 추가하세요.");
  }

  // 최근 대화 가져오기
  const recentInteractions = getRecentInteractions(3);
  ...
  if (!newTopic && recentInteractions.length > 0) {
    ...
    historyText = recentInteractions.slice(0, 2).map(interaction => 
      `사용자: ${clip(interaction.user_text, 300)}\n알파: ${clip(interaction.model_text, 300)}`
    ).join('\n\n');
  }
...
중요: 사용자의 현재 질문에만 집중해서 답변하세요. 이전 대화와 관련 없는 새로운 주제라면 이전 대화는 무시하세요.`;
  ...
  try {
    ...
    setImmediate(() => {
      try {
        saveLearningContext("conversation", `사용자: ${user} | AI: ${reply}`, 1);
        learnConversationPattern(user, reply);
      } catch (error) {
...
// ==================== 일정 자연어 파싱 (강화) ====================
export async function parseScheduleFromText(text: string): Promise<{
  title: string;
  date: string;
  time: string;
  description?: string;
...
// ==================== 웹 검색 결과 요약 (강화) ====================
export async function summarizeSearchResults(query: string, results: any[]): Promise<string> {
  if (!results || results.length === 0) {
    return "조사 결과, 해당 정보를 찾을 수 없었습니다.";
  }

  const resultsText = results.map((r, i) => 
    `${i + 1}. ${r.title || '제목 없음'}: ${r.content || r.text || ''}`
  ).join('\n');
...
// ==================== 파일/폴더 퍼지 검색 (강화) ====================
export async function fuzzyMatchFile(query: string, fileList: string[]): Promise<string | null> {
...
다른 설명 없이 경로만 반환하세요.`;
  ...
  try {
    ...
    // 응답에서 유효한 경로 찾기
    const matchedPath = fileList.find(f => 
      f.toLowerCase().includes(responseText.toLowerCase()) || 
...
// ==================== 감정 분석 (TTS용 강화) ====================
export async function analyzeEmotionForTTS(text: string): Promise<{
