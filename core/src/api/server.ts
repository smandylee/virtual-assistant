...
// Python 스크립트 실행 함수
async function runSpeakerAuth(command: string, args: string[] = []): Promise<any> {
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(process.cwd(), "voice-auth", "speaker_auth.py");
    const pythonCmd = process.platform === "win32" ? "python" : "python3";
    
    const proc = spawn(pythonCmd, [scriptPath, command, ...args], {
      cwd: process.cwd(),
...
app.post("/voice-auth/enroll", upload.array("audio", 5), async (req, res) => {
  try {
    ...
    // 임시 파일로 저장
    const tempDir = path.join(os.tmpdir(), "voice-auth-enroll");
    await fs.mkdir(tempDir, { recursive: true });
    
    const tempPaths: string[] = [];
    for (let i = 0; i < files.length; i++) {
      const tempPath = path.join(tempDir, `sample_${i}.wav`);
      await fs.writeFile(tempPath, files[i].buffer);
...
app.post("/voice-auth/verify", upload.single("audio"), async (req, res) => {
  try {
    ...
    // 임시 파일로 저장
    const tempPath = path.join(os.tmpdir(), `verify_${Date.now()}.wav`);
...
// 화자 검증 함수 (내부 사용)
async function verifySpeaker(audioBuffer: Buffer): Promise<{ verified: boolean; similarity?: number; error?: string }> {
  if (!voiceAuthEnabled) {
...
app.post("/chat", async (req, res) => {
  ...
  (global as any).lastInteractionTime = Date.now();
      ...
      if (toolCall && toolCall.tool) {
        ...
        switch (toolCall.tool) {
          case 'open_folder':
            ...
            if (!folderPath || folderPath === '' || folderPath.includes('WW') || folderPath.includes('₩')) {
              ...
              try {
                ...
                if (searchResult.length > 0) {
                  ...
                  const fuzzyResult = await fuzzyMatchFile(message, searchResult);
                  if (fuzzyResult) {
                    folderPath = fuzzyResult;
                    console.log('퍼지 검색으로 찾은 폴더:', folderPath);
                  } else {
                    // 퍼지 매칭 실패 시 일반 검색
                    const directSearch = await tools.search_files.execute({ 
            ...
            toolResult = await tools.open_folder.execute({ path: folderPath });
            toolSuccess = true;
            break;
          ...
          case 'open_file':
            ...
            toolResult = await tools.open_file.execute({ path: filePath });
            toolSuccess = true;
            break;

          case 'execute_command':
            toolResult = await tools.execute_command.execute({ command: toolCall.input, timeout: 30000 });
            toolSuccess = true;
            break;

          case 'search_files':
            toolResult = await tools.search_files.execute({ 
          ...
          case 'add_schedule':
            ...
            try {
              ...
              try {
                ...
                scheduleData = await parseScheduleFromText(toolCall.input || message);
              }
              
              if (scheduleData && scheduleData.title && scheduleData.date) {
                toolResult = await tools.add_schedule.execute({
          ...
          case 'get_schedules':
            ...
            try {
              ...
              toolResult = await tools.get_schedules.execute({
                date: queryData.date,
          ...
          case 'delete_schedule':
            ...
            try {
              ...
              toolResult = await tools.delete_schedule.execute({
                id: deleteData.id
              });
              toolSuccess = true;
            } catch (error) {
          ...
          case 'check_reminders':
            // 리마인더 체크
            toolResult = await tools.check_reminders.execute();
            toolSuccess = true;
            break;

          case 'cleanup_expired_schedules':
            // 지난 일정 자동 정리
            toolResult = await tools.cleanup_expired_schedules.execute();
            toolSuccess = true;
            break;
          ...
          case 'launch_steam_game':
            ...
            try {
              ...
              toolResult = await tools.launch_steam_game.execute({
          ...
          case 'launch_program':
            ...
            try {
              ...
              toolResult = await tools.launch_program.execute({
                programName
              });
              toolSuccess = true;
          ...
          case 'youtube_search':
            try {
              ...
              toolResult = await tools.youtube_search.execute({ query: ytSearchQuery, maxResults: 5 });
              toolSuccess = true;
            } catch (error: any) {
          ...
          case 'youtube_play':
            try {
              ...
              toolResult = await tools.youtube_play.execute({
          ...
          case 'youtube_channel_videos':
            try {
              ...
              const searchResult = await tools.youtube_search.execute({ 
                query: searchName, 
                maxResults: 5 
              });
              ...
              if (searchResult.success && searchResult.results?.length > 0) {
                ...
                if (message.includes('틀어') || message.includes('재생') || message.includes('보여')) {
                  ...
                  const playResult = await tools.youtube_play.execute({ videoId });
          ...
          case 'youtube_trending':
            try {
              ...
              toolResult = await tools.youtube_trending.execute({
...
async function speechToText(audioBuffer: Buffer, filename: string): Promise<string> {
  try {
...
app.post("/text-to-speech", async (req, res) => {
  try {
    const { 
      text, 
      voice = 'rUSWM861uoIpt6gT6Vpt',  // 사용자 커스텀 음성
        ...
        switch (toolCall.tool) {
          case 'open_folder':
            ...
            if (!folderPath2 || folderPath2 === '' || folderPath2.includes('WW') || folderPath2.includes('₩')) {
              try {
                const searchResult3 = await tools.search_files.execute({ 
                  query: '', 
                  dir: "C:\\Users\\User\\Desktop",
                  maxResults: 50
                });
                ...
                if (searchResult3.length > 0) {
                  const fuzzyResult2 = await fuzzyMatchFile(message, searchResult3);
                  if (fuzzyResult2) {
                    folderPath2 = fuzzyResult2;
                    console.log('퍼지 검색으로 찾은 폴더:', folderPath2);
                  } else {
                    const directSearch2 = await tools.search_files.execute({ 
                      query: message.replace(/[^\w\s가-힣]/g, '').trim(), 
                      dir: "C:\\Users\\User\\Desktop" 
                    });
            ...
            toolResult = await tools.open_folder.execute({ path: folderPath2 });
            toolSuccess = true;
            break;
          ...
          case 'youtube_search':
            try {
              ...
              toolResult = await tools.youtube_search.execute({ query: ytSearchQ, maxResults: 5 });
              toolSuccess = true;
            } catch (error: any) {
              toolError = `YouTube 검색 실패: ${error.message}`;
            }
          ...
          case 'youtube_channel_videos':
            try {
              ...
              // 🔥 YouTube API 시도, 실패시 브라우저로 직접 열기
              const searchRes = await tools.youtube_search.execute({ 
                query: searchName2, 
                maxResults: 5 
              });
              ...
              if (searchRes.success && searchRes.results?.length > 0) {
                ...
                if (message.includes('틀어') || message.includes('재생') || message.includes('보여')) {
                  ...
                  const playRes = await tools.youtube_play.execute({ videoId: vidId });
...
app.post("/backup", (req, res) => {
  try {
    ...
    const backupPath = backupDatabase(customPath);
...
app.post("/cleanup/interactions", (req, res) => {
  try {
    ...
    const deleted = cleanupOldInteractions(daysToKeep);
...
app.post("/cleanup/context", (req, res) => {
  try {
    ...
    const deleted = cleanupLowImportanceContext(importanceThreshold);
    res.json({ success: true, deleted });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
app.post("/cleanup/patterns", (req, res) => {
  try {
    const deleted = cleanupDuplicatePatterns();
    res.json({ success: true, deleted });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
function generateNaturalResponse(toolResult: any, message: string): string {
  // YouTube 관련
  ...
  if (toolResult.schedules) {
    ...
    const scheduleList = toolResult.schedules.slice(0, 3).map((s: any) => 
...
app.get("/youtube/search", async (req, res) => {
  try {
    ...
    const result = await tools.youtube_search.execute({ query, maxResults });
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
app.post("/youtube/play", async (req, res) => {
  try {
    ...
    const result = await tools.youtube_play.execute({ query, videoId, url });
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
app.get("/youtube/trending", async (req, res) => {
  try {
    const regionCode = (req.query.region as string) || 'KR';
    const maxResults = parseInt(req.query.maxResults as string) || 10;
    const category = req.query.category as string;
    
    const result = await tools.youtube_trending.execute({ regionCode, maxResults, category });
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
app.get("/youtube/channel", async (req, res) => {
  try {
    ...
    const result = await tools.youtube_channel_videos.execute({ channelName, channelId, maxResults });
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
app.get("/youtube/video/:videoId", async (req, res) => {
  try {
    ...
    const result = await tools.youtube_video_info.execute({ videoId });
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
app.post("/youtube/voice-play", upload.single('audio'), async (req, res) => {
  try {
    ...
    const result = await tools.youtube_play.execute({ query: cleanQuery });
...
// 연결된 클라이언트들을 저장
const reminderClients: Set<any> = new Set();
...
function broadcastReminder(data: any) {
...
app.get("/reminders/check", async (_req, res) => {
  try {
    const result = await tools.check_reminders.execute();
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
const notifiedReminders: Set<string> = new Set();
...
// 마지막 대화 시간 추적 (전역)
(global as any).lastInteractionTime = (global as any).lastInteractionTime || Date.now();
...
**말 거는 상황:** ${
  context.type === 'greeting' ? '시간대에 맞는 인사 (아침/점심/저녁/밤)' :
...
- 단순 인사만 하지 말고 무언가 제안하거나 질문하세요`;
  ...
  try {
    const response = await chatWithGemini(systemPrompt, "주인에게 먼저 말을 거세요.");
...
// 오늘의 일정 가져오기
async function getTodaySchedules(): Promise<any[]> {
  try {
    const result = await tools.get_schedules.execute({ 
...
// 시간대별 인사 체크 (AI 생성)
async function getTimeBasedGreeting(): Promise<{ message: string; type: string } | null> {
  const now = new Date();
  const hour = now.getHours();
...
async function getIdleMessage(): Promise<{ message: string; type: string } | null> {
  const lastTime = (global as any).lastInteractionTime || Date.now();
...
// 랜덤 대화 주제 (AI 생성)
async function getRandomConversation(): Promise<{ message: string; type: string } | null> {
...
// 능동적 AI 메시지 브로드캐스트
function broadcastProactiveMessage(data: { message: string; type: string }) {
  broadcastReminder({
    type: 'proactive',
    subtype: data.type,
...
setInterval(async () => {
  ...
  if (reminderClients.size === 0) return; // 연결된 클라이언트 없으면 스킵
